...
def random_weights(sizes):
    ...
    """
    return [xavier_initialization(sizes[i], sizes[i+1]) for i in range(len(sizes)) if i <= (len(sizes) - 2)]
...
def zeros_weights(sizes):
    ...
    """
    return [np.zeros((sizes[i], sizes[i+1])) for i in range(len(sizes)) if i <= (len(sizes) - 2)]
